...
public class DaoFolderComunicacao {
    ...
    /**
     * Verifica se um folder pertence a um usuário específico.
     * @param folderId O ID do folder a ser verificado.
     * @param fkUsuarioId O ID do usuário.
     * @return true se o folder pertencer ao usuário, false caso contrário.
     */
    public boolean verificarFolderDoUsuario(int folderId, int fkUsuarioId) {
        String sql = "SELECT COUNT(*) FROM FolderComunicacao WHERE id = ? AND fk_usuario_id = ?";
        
        try (Connection conn = ConexaoDB.getConexao();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            
            stmt.setInt(1, folderId);
            stmt.setInt(2, fkUsuarioId);
            ...
            System.err.println("DAO ERRO: Falha ao verificar se o folder pertence ao usuário.");
            e.printStackTrace();
        }
        return false;
    }
