    ...
    <script>
        ...
        foldersContainer.addEventListener('click', (event) => {
            ...
            if (modoExclusaoAtivo) {
                ...
                // Usando crases (`) para permitir a interpolação da variável
                if (confirm(`Tem certeza que deseja excluir o folder "${folderCard.querySelector('h3').textContent}"? A ação não pode ser desfeita.`)) {
                    ...
                    // ** CORREÇÃO CRÍTICA AQUI **
                    // Usando crases (`) para montar a URL corretamente
                    fetch(`${contextPath}/ExcluirFolderServlet?id=${folderId}`, { method: 'POST' })
                    .then(response => {
                        if (response.ok) {
                            folderCard.remove();
