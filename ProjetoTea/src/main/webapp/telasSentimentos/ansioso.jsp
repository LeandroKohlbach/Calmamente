...
<head>
    ...
    <style>
        ...
        body { font-family: var(--fonte-corpo), sans-serif; color: var(--cor-texto-suave); line-height: 1.6; overflow-x: hidden; background-image: linear-gradient(to bottom, #E0F7FA, #CCEEFF); min-height: 100vh; display: flex; flex-direction: column; position: relative; }
        main { flex-grow: 1; display: flex; justify-content: center; align-items: center; padding: 2rem; position: relative; z-index: 1; }

        .star { position: absolute; background-color: var(--cor-estrela-luz); border-radius: 50%; box-shadow: 0 0 5px var(--cor-estrela-luz), 0 0 10px var(--cor-estrela-luz); animation: twinkle 5s linear infinite; z-index: 0; }
        ...
        .content-wrapper h1 { font-family: var(--fonte-titulo); font-size: 3.2rem; font-weight: 700; color: var(--cor-principal-calma); margin-bottom: 1.5rem; }
        .content-wrapper .intro-text { font-size: 1.2rem; color: var(--cor-texto-suave); margin-bottom: 1rem; max-width: 600px; margin-left: auto; margin-right: auto; }
        .activity-area { margin-bottom: 1.5rem; padding: 1.5rem 0; border-radius: 20px; min-height: 250px; display:flex; flex-direction: column; justify-content:center; align-items:center; position: relative; }
        ...
        .lumi-body { width: 180px; height: 160px; background: radial-gradient(circle at 50% 100%, #fff, var(--cor-lumi-corpo) 70%); border-radius: 50% 50% 45% 45%; position: relative; box-shadow: inset 0 -10px 20px rgba(0,0,0,0.05); }
        .lumi-eyes { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -60%); display: flex; gap: 30px; }
        .lumi-eye { width: 15px; height: 20px; background-color: var(--cor-lumi-olhos); border-radius: 50%; animation: blink 7s ease-in-out infinite; }
        .lumi-mouth {
            position: absolute;
            bottom: 30%;
            left: 50%;
        ...
        .lumi-shadow { width: 120px; height: 20px; background-color: var(--cor-lumi-sombra); border-radius: 50%; margin: 20px auto 0; filter: blur(8px); opacity: 0.7; animation: breathe-shadow 8s ease-in-out infinite; }

        @keyframes blink { 0%, 90%, 100% { transform: scaleY(1); } 95% { transform: scaleY(0.1); } }
        @keyframes breathe-shadow { 0% { transform: scale(1); opacity: 0.7; } 50% { transform: scale(0.9); opacity: 0.5; } 100% { transform: scale(1); opacity: 0.7; } }
        ...
        footer .links a { color: var(--cor-texto-claro); text-decoration: none; transition: color 0.3s ease, transform 0.2s ease; padding: 0.5rem 0; font-weight: 600; }
        footer .links a:hover { color: var(--cor-secundaria-acao); transform: translateY(-2px); }
        .nav-buttons { position: relative; }
        .user-profile-button { background-color: var(--cor-principal-calma); color: var(--cor-texto-claro); border: none; border-radius: 50%; width: 50px; height: 50px; font-size: 1.8rem; cursor: pointer; display: flex; justify-content: center; align-items: center; box-shadow: 0 3px 6px rgba(0,0,0,0.2); }
        ...
        .dropdown-content .user-info strong { color: var(--cor-principal-calma); }
        ...
        .dropdown-content a .fa-sign-out-alt { color: #FF6347; }
    </style>
</head>
<body>
    ...
    <main>
        <div class="content-wrapper">
            ...
            <p class="intro-text">Quando a ansiedade chegar e deixar o coração acelerado, respirar devagar com um amigo ajuda a trazer a tranquilidade de volta. Vamos tentar juntos?</p>
            ...
            <div class="action-box">
                ...
                <p>Quando o Lumi crescer, puxe o ar pelo nariz. Quando ele encolher, solte o ar devagar pela boca.</p>
