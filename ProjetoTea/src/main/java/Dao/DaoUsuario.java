...
public class DaoUsuario {
	...
	public static Usuario insert(Usuario usuario) {
		try {
			...
			// Mantém "Usuario" com 'U' maiúsculo para consistência
	...
	public boolean emailJaExiste(String email) throws Exception {
        // CORRIGIDO: Alterado de "usuarios" para "Usuario" para consistência
        ...
        try (Connection conn = ConexaoDB.getConexao();
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    return rs.getInt(1) > 0; // Se count > 0, o email existe
                }
            }
        }
        return false; // Por padrão, não existe
    }
}